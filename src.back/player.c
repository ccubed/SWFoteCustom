...
void do_level( CHAR_DATA * ch, char *argument )
{
   ...
   for( ability = 0; ability < MAX_ABILITY; ability++ )
      if( ability != FORCE_ABILITY )
         ch_printf( ch, "&G%-15s Level&W: %-3d    &GMax&W: %-3d    &GExp&W: %-10d     &GNext&W: %-10d&W\n\r",
                    ability_name[ability], ch->skill_level[ability], ch->max_level[ability], ch->experience[ability],
                    exp_level( ch->skill_level[ability] + 1 ) );
}
