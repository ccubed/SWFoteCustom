...
void do_give( CHAR_DATA * ch, char *argument )
{
   ...
   if( is_number( arg1 ) )
   {
      ...
      ch->gold -= amount;
      victim->gold += amount;
      sprintf( buf, "You receive %d %s from $n.", amount, ( amount > 1 ) ? "credits" : "credit" );
/*
...
*/
      ...
      sprintf( buf, "You give $N %d %s", amount, ( amount > 1 ) ? "credits." : "credit." );
   ...
   if( arg1[0] == '\0' )
   {
      if( auction->item != NULL )
      {
         ...
         if( IS_IMMORTAL( ch ) )
         {
            ...
            send_to_char( buf, ch );
            sprintf( buf, "Time left in round: %d seconds\n\r", auction->pulse / 4 );
            send_to_char( buf, ch );
         }
         return;
      }
...
/*	    ch_printf( ch, "Bid: %d\n\r",newbet);	*/
         ...
         sprintf( buf, "&WNew bidder: &Y%d &Wcredits for %s.\n\r", newbet, auction->item->short_descr );
         talk_auction( buf );
         return;
...
*/
         ...
         case ITEM_GOGGLES:
            ...
            sprintf( buf, "&WNew item: %s&W at &Y%d&W credits.", obj->short_descr, auction->starting );
            talk_auction( buf );
