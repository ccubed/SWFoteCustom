      ...
      for( obj = keeper->last_carrying; obj; obj = obj->prev_content )
      {
         if( obj->wear_loc == WEAR_NONE && can_see_obj( ch, obj ) )
         {
            ...
            if( ( cost = get_cost( ch, keeper, obj, TRUE ) ) > 0 && ( arg[0] == '\0' || nifty_is_name( arg, obj->name ) ) )
            {
               ...
               {
                  found = TRUE;
                  send_to_char( "[Price] {ref} [Type] Item\n\r", ch );
               }

               ch_printf( ch, "[%5d] {%3d} [%s] %s%s.\n\r",
                          cost, oref, item_type_name(obj), capitalize( obj->short_descr ),
...
void do_sell( CHAR_DATA * ch, char *argument )
{
   ...
   //give tax back to local clan
   if (taxrate > 0) ch->in_room->area->planet->governed_by->funds += taxrate;
...
void do_value( CHAR_DATA * ch, char *argument )
{
   ...
   if (taxrate > 0){
	   ...
	   sprintf(buf, "$n tells you 'I'll give you %d credits for $p after taxes.'", cost);
...
void do_buyvendor(CHAR_DATA * ch, char *argument){
	...
	if (ch->gold < 200000){
		...
		act(AT_TELL, "&RYou need 200000 credits for a vendor deed.&w", ch, NULL, Keeper, TO_CHAR);
		return;
