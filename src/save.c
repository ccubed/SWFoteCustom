...
bool load_char_obj( DESCRIPTOR_DATA * d, char *name, bool preload, bool hotboot )
{
   ...
   ch->hasDeed = 0; // has a vendor deed
   ch->vendorNum = 0; // total number of vendors owned
   for (i = 0; i < 10; i++)
	   ch->vendors[i] = 0; // zero out all vendor vnums
#ifdef IMC
   imc_initchar( ch );
#endif
   found = FALSE;
...
void fread_char( CHAR_DATA *ch, FILE *fp, bool preload, bool hotboot )
{
    ...
    int sn, extra;
    char acrobuf[MSL];
	int vtrack = 0; //track player vendor index
	...
	case 'H':
		...
		KEY("hasDeed", ch->hasDeed, fread_number(fp));
